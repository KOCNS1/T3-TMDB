...
const ContentDetails = ({ type, id }: Props) => {
  const { data, isLoading } = trpc.useQuery([
    `tmdb.${type}.getDetails`,
    { appendToResponse: ["videos"], id: +id },
  ]);
  ...
  return (
    <div>
      <div className="flex gap-6 bg-neutral rounded-lg p-4">
        <div className="w-1/4">
          <Image
        ...
        <div className="flex flex-col gap-10 text-white w-3/4 justify-center">
          <div>
            ...
            <div className="flex justify-start gap-2 mt-1 font-light">
              {/* <p *ngFor="let genre of data.genres; let i = index">
            {{ genre.name }}{{ data.genres.length - 1 !== i ? ',' : '' }}
          </p> */}
            </div>
          </div>
          ...
          <div className="flex items-center justify-start gap-9">
            ...
            <div>
              <label
                htmlFor="my-modal-4"
                className="btn modal-button btn-outline border-white text-white gap-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
